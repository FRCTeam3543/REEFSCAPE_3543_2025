 ...
 */
public class EuroVision extends Command {
    ...
    private static final int alignLeft = 2;
    private static final int alignRight = 3;
     ...
     *                          wall.
     */
    public EuroVision(SwerveSubsystem swerve, DoubleSupplier vX, DoubleSupplier vY, DoubleSupplier headingHorizontal,
            DoubleSupplier headingVertical, Limelight LL) {
        this.swerve = swerve;
        this.vX = vX;
        this.vY = vY;
    ...
    int getTargetPosition() {
        if (Constants.OperatorConstants.driverXbox.button(7).getAsBoolean()) {
            return alignCenter;
        } else if (Constants.OperatorConstants.driverXbox.button(6).getAsBoolean()) {
            return alignRight;
        } else if (Constants.OperatorConstants.driverXbox.button(5).getAsBoolean()) {
            return alignLeft;
        }
        return NONE;
    }
    ...
    public void execute() {
        ...
        switch (getTargetPosition) {
            case alignRight:
                NetworkTableInstance.getDefault().getTable("limelight").getEntry("fiducial_offset_set")
                        .setNumberArray(new Number[] { 0.164338, 0, 0 });
                ...
                if (initRotation) {
                    if (headingHorizontal.getAsDouble() == 0 && headingVertical.getAsDouble() == 0) {
                        ...
                        desiredSpeeds = swerve.getTargetSpeeds(0, 0, firstLoopHeading.getSin(),
            ...
            case alignCenter:
                NetworkTableInstance.getDefault().getTable("limelight").getEntry("fiducial_offset_set")
                        .setNumberArray(new Number[] { 0, 0, 0 });
                // Get the desired chassis speeds based on a 2 joystick module.
                desiredSpeeds = swerve.getTargetSpeeds(0, 0,
                        LL.giveX(),
                        0);
